...
#include <Wire.h>
const int MPU_ADDR = 0x68; // MPU_ADDR6050 I2C address
...
int c = 0;
int orientation = 0; //0 is center, 1 is right, -1 is left
void setup() {
  Serial.begin(9600);
  Wire.begin();                      // Initialize comunication
  Wire.beginTransmission(MPU_ADDR);       // Start communication with MPU_ADDR6050 // MPU_ADDR=0x68
...
void loop() {
  ...
  Wire.endTransmission(false);
  Wire.requestFrom(MPU_ADDR, 7*2, true); // Read 4 registers total, each axis value is stored in 2 registers
